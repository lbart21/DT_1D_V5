...
class SinglePhaseSingleSpeciesNonreactiveQuadraticNozzle(SingleInlet1DMeshObject):
    __slots__ = []
    def __init__(self, geometry, n_cells, init_flow_state, restart_flag, recon_scheme, \
                        limiter, recon_props, update_from, flux_scheme, comp_label, reverse_direction_for_ghost_cells=False) -> None:
        super().__init__(n_cells, reverse_direction_for_ghost_cells)
        # geometry = [D_L, D_R, L]
        self.component_labels = [comp_label]

        self.initialise_cells(geometry = geometry, n_cells = n_cells, comp_label = comp_label, \
        ...
        self.initialise_interfaces(geometry = geometry, n_cells = n_cells, \
                                    ...
                                    flux_scheme = flux_scheme, init_flow_state = init_flow_state)

    def initialise_cells(self, geometry, n_cells, comp_label, init_flow_state, restart_flag):
        [_, _,  L] = geometry
    ...
    def initialise_interfaces(self, geometry, n_cells, recon_scheme, limiter, recon_props, \
        ...
        for interface in range(n_cells + 1):
            ...
            interface_object = SinglePhaseSingleSpeciesInterface(interface_id = interface, \
                                    recon_scheme = recon_scheme, limiter = limiter, \
                                    recon_props = recon_props, update_from = update_from, \
    ...
    def find_diameter_at_x(self, geometry, x):
        [D_L, D_R, L] = geometry
        return D_L + (D_R - D_L) * x / L + (D_R - D_L) * x * (L - x) / L ** 2.0
