...
class SinglePhaseMultiSpeciesFiniteRateReactivePipe(SingleInlet1DMeshObject):
    __slots__ = []
    def __init__(self, n_cells, geometry, comp_label, init_flow_state, restart_flag, \
                        shared_reactor_object, flux_scheme, recon_scheme, recon_props, \
                        update_from, limiter, reverse_direction_for_ghost_cells = False) -> None:
        ...
        self.initialise_cells(comp_label = comp_label, geometry = geometry, n_cells = n_cells, \
        ...
        self.initialise_interfaces(geometry = geometry, n_cells = n_cells, \
                                    ...
                                    update_from = update_from, limiter = limiter)
    
    def initialise_cells(self, comp_label, geometry, n_cells, init_flow_state, \
                                shared_reactor_object, restart_flag):
        [D, L] = geometry
    ...
    def initialise_interfaces(self, geometry, n_cells, init_flow_state, flux_scheme, recon_scheme, \
                                    recon_props, update_from, limiter):
        [D, L] = geometry
        for interface in range(n_cells + 1):
            ...
            interface_object = SinglePhaseMultiSpeciesInterface(\
