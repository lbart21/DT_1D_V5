...
class SinglePhaseMultiSpeciesNonreactivePipe(SingleInlet1DMeshObject):
    __slots__ = []
    def __init__(self, n_cells, geometry, init_flow_state, flux_scheme, recon_scheme, recon_props, \
                        limiter, update_from, comp_label, restart_flag, \
        ...
        #                               p, T, massf and vel_x values for each cell
        # geometry = [D, L]

        self.component_labels = [comp_label]

        self.initialise_cells(geometry = geometry, init_flow_state = init_flow_state, \
                                comp_label = comp_label, n_cells = n_cells, \
                                restart_flag = restart_flag)

        self.initialise_interfaces(n_cells = n_cells, geometry = geometry, \
                                    init_flow_state = init_flow_state, flux_scheme = flux_scheme, \
                                    recon_scheme = recon_scheme, update_from = update_from, \
                                    limiter = limiter, recon_props = recon_props)

    def initialise_cells(self, geometry, init_flow_state, comp_label, n_cells, restart_flag):
        [D, L] = geometry
    ...
    def initialise_interfaces(self, n_cells, geometry, init_flow_state, flux_scheme, recon_scheme, \
