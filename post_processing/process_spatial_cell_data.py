...
class ProcessSpatialCellData():
    def __init__(self, spatial_cell_data_file) -> None:
        cwd = os.getcwd()
        file_length = sum(1 for line in open(cwd + "/data/" + spatial_cell_data_file))
        file = open(cwd + "/data/" + spatial_cell_data_file)
        variable_flag = -1
        data_start = False
